function getAllButtons(isAccept) {
    try {
        const elementZIndex = getAllDivsWithZIndex();
        let targetElements = [];
        elementZIndex.forEach((ele) => {
            targetElements = [...targetElements, ...getAcceptRejectElement(ele, isAccept)]
        })
...
function getAcceptRejectElement(ele, isAccept) {
    const targetElements = [];
    const allElements = ele.getElementsByTagName("*");
    for (let c = 0; c < allElements.length; c++) {
        let text = allElements[c]?.innerText;
        if (isAccept) {
            if (cookieAcceptWords.includes(text?.toLowerCase?.()) && !allElements[c]?.children?.length > 0) {
                targetElements.push(allElements[c]);
            }
        } else {
            if (cookieRejectWords.includes(text?.toLowerCase?.()) && !allElements[c]?.children?.length > 0) {
                targetElements.push(allElements[c]);
            }
        }
    }
...
function getAllDivsWithZIndex() {
    ...
    for (let c = 0; c < tags.length; c++) {
        ...
        if (zIndex > 1 || zIndex === 'auto') {
            let containsCSS = tags[c].className
            let isTrue = false;
            if (typeof containsCSS === "string") {
                isTrue = cookiesSelectorsCSS1.includes(containsCSS) ||
                    cookiesSelectorsCSS2.includes(containsCSS) ||
                    cookiesSelectorsCSS3.includes(containsCSS) ||
                    cookiesSelectorsCSS4.includes(containsCSS)
            } else if (typeof containsCSS === "object" && containsCSS.length) {
                isTrue = containsCSS.some(r => cookiesSelectorsCSS1.includes(r.trim())) ||
                    containsCSS.some(r => cookiesSelectorsCSS2.includes(r.trim())) ||
                    containsCSS.some(r => cookiesSelectorsCSS3.includes(r.trim())) ||
                    containsCSS.some(r => cookiesSelectorsCSS4.includes(r.trim()))
            }
            containsCSS = tags[c].id;
            if (containsCSS) {
                isTrue = cookiesSelectorsCSS1.toLowerCase().includes(containsCSS.trim().toLowerCase()) ||
                    cookiesSelectorsCSS2.toLowerCase().includes(containsCSS.trim().toLowerCase()) ||
                    cookiesSelectorsCSS3.toLowerCase().includes(containsCSS.trim().toLowerCase()) ||
                    cookiesSelectorsCSS4.toLowerCase().includes(containsCSS.trim().toLowerCase())
            }
            if (isTrue === true) {
                divs.push(tags[c]);
            }
...
function acceptAllCookies() {
    const elements = getAllButtons(true);
...
function rejectAllCookies() {
    const elements = getAllButtons(false);
    for (let c = 0; c < elements.length; c++) {
        if (elements[c].click) {
            elements[c].click()
        }
...
function hideCookiePopup() {
    alert("hide cookie function got triggered")
    chrome.runtime.sendMessage({ hideCookiePopup: true }, function (response) {
        //
    });
}


const cookiesSelectorsCSS1 =
    `
html.cookie-notification-active, html.consent-bar-push-large, body.consent-bar-push-large, body.cookiepush, body.with-eu-cookie-guideline, body.gh-cookieb-active #gh.gh-cookieb-active, html.npo_cookieBar body, body.cookiewall, body.cookie-guard, body.with-cookie-hint, body.with-cookie-bar, html.has-cookie-bar, body.with-cookie-hint, body.privacypopup-open,
#cookiesPanel ~ #header, #CpHTMLCookieBar_siteWrapper, body.show-cookie-policy-info,
body.needsCookieAcceptance, body.cookie-not-set, body.cookie_privacy_info, html.cookie body, body.has-cookie-banner, body.with-cookie-alert, body.body--is-alerted, body.ct-ultimate-gdpr-cookie-topPanel-padding,
html.with-cookie-hint, body.gh-cookieb-active #gh-gb.gh-cookieb-active,
html.with-cookie-popup, html.with-cookie-popup body, body.page_cookieconfirmation, body.cookieoverlay-is-open, body.cookiewall-active, body.qc-cmp-ui-showing, body.freezePage, html.sp-message-open, body.cookie_banner_prevent_scroll, body.cli-barmodal-open,
body.cookie-consent-checking, html.sp-message-open,
#cookieNoticeModal.vrm-reveal[style*="block"], div[id^="xsp_message_container_"],
.ot-sdk-row,
#stickyCookieBar,
cookie-policy,
...
#mmmmm-cookies,
#cookie-consent:not(.content):not([class*="fancybox"]),
#cookienote,
#cc-notification,
...
.cookie-popup:not(body):not(html):not(#colorbox),
#FT_Cookies,
#policyNotice,
...
.accept-cookies:not(body):not(html):not(button):not(.btn):not(a),
...
#MSoftCookieWarn,
.cookie-accept:not(body):not(html):not(.btn):not(button),
...
#ccc > #ccc-notify,
div[class*="CookieBanner"]:not([class*="page"]):not(.app),
.card-cookie-notice,
#smart-cmp-host,
#gdprsticky,
...
notice,
message type-modal,
gravitoCMP-modal gravitoCMP-modal--layer1,
gravitoCMP-background-overlay,
ark-cookiebar,
ark-cookiebar-content,
#uc-center-container.sc-jrQzAO hvzrKj,
.cookies-acceptance-wrapper-common`
...
const cookieAcceptWords = [
    ...
    "i'm ok with this",
    "i consent",
    "hide this message about cookies",
    "do not show this message again",
    "close this message",
    "okay i accept",
    "i'm ok with that",
    "i accept the cookies from this site",
    "hide message",
    ...
    "i’m fine with cookies",
    "i'm happy with this",
    "i am happy to accept the cookies",
    "i am fine with this",
    ...
    "accept the use of all recommended cookies",
    "accept + close",
    "✖ hide",
    "✔ that's fine",
    "✔ continue",
    ...
    "yes i understand!",
    "yes i understand",
    "yes i agree to use cookies",
    "yes i accept all cookies",
    "yes - got it hide the message",
    "whatever!",
    "we use cookies",
    ...
    "save",
    "i understand",
    "please dont show this message again",
    "opt in",
    "okie dokie",
    "okay, i agree",
    "okay, agree",
    ...
    "im fine with this",
    "ignore",
    "i've read and understood this message",
    "i'm okay with this",
    "i'm ok",
    "i'm happy with cookies",
    "i'm happy to proceed",
    "i'm happy to continue",
    "i'm happy to accept the cookies",
    "i'm fine with this [x]",
    "i'm cool!",
    "i have read and understood",
    "i consent to use of cookies",
    "i consent to the use of cookies on this website",
    "i am fine with it",
    "i agreee",
    "i agree!",
    "i agree with it!",
    "i agree don't show me this again",
    ...
    "great",
    "got it! thanks",
    "got it! agree and proceed",
    "got it save settings",
    "got it dont show again",
    "enable all cookies and save",
    "enable all cookies",
    "dont show this message again",
    "dont show me this message anymore",
    ...
    "close!",
    "close window",
    "close this message and continue",
    "close cookie acceptance message",
    ...
    "click to accept",
    "click here to dismiss this notice",
    "click here to continue",
    "click here to accept cookies",
    "click here to accept",
    "cleick here to agree to our policy",
    "carry on",
    "Allow essential and optional cookies",
    "allow cookies and stop asking",
    "allow and continue",
    "aknowledge & close",
    "aknowledge",
    "agree and proceed",
    ...
    "accept tracking",
    "accept standard settings",
    "accept recommended cookies",
    "accept our cookies",
    "accept my preferences",
    "accept marketing-cookies",
    "accept cookies and our terms of use",
    "accept cookies and continue",
    "accept cookies and close this notice",
    "accept cookies and close this message",
    "accept cookie",
    "accept and dismiss",
    "accept and close the cookie policy",
    "accept and close about cookies",
    "accept all & continue",
    "OK, Got it"
];

const cookieRejectWords = [
    "decline",
    ...
    "refuse",
    "no to all cookies",
    "no not accept cookies - close",
    "no cookies please",
    "i refuse",
    "Disable All",
    "i don't accept the cookies from this site",
    "i decline",
    "do not accept cookies - close",
    "disallow cookies",
    "disabled",
    "disable them here",
